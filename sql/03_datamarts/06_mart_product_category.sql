-- Mart: mart_product_category
-- Domain: Product / Category Management
-- Grain: 1 row = 1 purchase month + 1 product category.
-- Main sources: fact_order_item_sales, fact_reviews, dim_product, dim_date,
-- dim_seller.
-- Purpose: analyze product/category revenue, sold items, freight, product
-- weight/volume, and review quality.
-- Duplication note: sales is aggregated at month/category grain first. Reviews
-- are joined through a distinct order-category bridge, so item-level sales rows
-- do not multiply order-level review rows. If an order contains multiple
-- categories, its review is allocated to each category present in that order.

DROP TABLE IF EXISTS mart.mart_product_category CASCADE;

CREATE TABLE mart.mart_product_category AS
WITH sales_base AS (
    SELECT
        d.year,
        d.month,
