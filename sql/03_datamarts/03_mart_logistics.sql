...
-- Grain: 1 row = 1 purchase month + 1 customer state + 1 order status.
-- Main sources: fact_order_delivery, dim_date, dim_customer, dim_order_status,
-- dim_geolocation.
-- Purpose: track delivery performance, late delivery rate, approval time,
-- delivery duration, and delay by customer region and order status.

DROP TABLE IF EXISTS mart.mart_logistics CASCADE;

CREATE TABLE mart.mart_logistics AS
SELECT
    d.year,
    d.month,
    d.month_name,
