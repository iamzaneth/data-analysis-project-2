...
-- Domain: Marketplace / Seller Management
-- Grain: 1 row = 1 purchase month + 1 seller + 1 product category.
-- Main sources: fact_order_item_sales, fact_order_delivery, fact_reviews,
-- dim_seller, dim_product, dim_date, dim_geolocation.
-- Purpose: evaluate seller performance by revenue, sold items, review score,
-- late delivery rate, and category contribution.
-- Duplication note: sales is aggregated first at seller/category/month grain.
-- Delivery and review metrics are calculated from a distinct order-seller-
-- category bridge before joining back to sales aggregates. If an order contains
-- multiple seller/category combinations, order-level review and delivery metrics
-- are allocated to each seller/category present in that order.

DROP TABLE IF EXISTS mart.mart_seller_performance CASCADE;

CREATE TABLE mart.mart_seller_performance AS
WITH sales_base AS (
    SELECT
        d.year,
        d.month,
...
delivery_by_seller_category_month AS (
    SELECT
        ...
        ROUND(
            ...
            / NULLIF(COUNT(DISTINCT bridge.order_id) FILTER (WHERE delivery.order_delivered_customer_date IS NOT NULL), 0),
            2
        ) AS late_rate_pct
...
review_by_seller_category_month AS (
    SELECT
        ...
        ROUND(100.0 * COUNT(r.review_key) FILTER (WHERE r.review_score <= 2) / NULLIF(COUNT(r.review_key), 0), 2) AS low_review_rate_pct
    FROM distinct_order_seller_category_month bridge
    LEFT JOIN dwh.fact_reviews r
