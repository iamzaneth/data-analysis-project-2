...
-- Suggested charts: category ranking bar chart, scatter plot for freight vs
-- review, treemap, monthly category trend, risk table.
-- Expected insights: high-value categories, freight-heavy categories, categories
-- with low review risk, and product size impact.
...
SELECT
    ...
    m.product_category_name_english AS product_category,
    SUM(m.total_orders) AS total_orders,
    ROUND(SUM(m.total_revenue)::NUMERIC, 2) AS total_revenue
FROM mart.mart_product_category m
JOIN top_categories t
    ON t.product_category_name_english = m.product_category_name_english
GROUP BY m.year, m.month, m.product_category_name_english
